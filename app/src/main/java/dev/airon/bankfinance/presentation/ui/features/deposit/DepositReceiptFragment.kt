...
class DepositReceiptFragment : Fragment()  {
    ...
    private fun initListener() {
        binding.btnConfirmationTransaction.setOnClickListener {
            val action = DepositReceiptFragmentDirections
                .actionDepositReceiptFragmentToHomeFragment()
    ...
    private fun getDeposit() {
        viewModel.getDeposit(args.idDeposit).observe(viewLifecycleOwner) { stateView ->
            when (stateView) {
                ...
                is StateView.Success -> stateView.data?.let { configData(it) }
                is StateView.Error -> {
    ...
    private fun configData(deposit: Deposit) {
        ...
        binding.textDateDeposit.text =
            GetMask.getFormatedDate(deposit.date, GetMask.DAY_MONTH_YEAR)
        binding.textHourDeposit.text =
            GetMask.getFormatedDate(deposit.date, GetMask.HOUR_MINUTE)
    }
